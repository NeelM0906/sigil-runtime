...
model = 'openai/text-embedding-3-small' if os.environ.get('OPENROUTER_API_KEY') else 'text-embedding-3-small'
emb = openai.embeddings.create(model=model, input=query).data[0].embedding
