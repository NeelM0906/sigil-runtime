...
app.get('/api/sessions', async (req, res) => {
  try {
    const { stdout } = await execAsync('curl -s "http://localhost:18789/api/sessions?limit=50&messageLimit=1" -H "Authorization: Bearer [REDACTED]"');
...
app.post('/api/spawn', async (req, res) => {
  try {
    ...
    const { stdout } = await execAsync(`curl -s -X POST "http://localhost:18789/api/sessions/spawn" -H "Authorization: Bearer [REDACTED]" -H "Content-Type: application/json" -d '${payload}'`);
    res.json(JSON.parse(stdout));
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
...
app.post('/api/sessions/send', async (req, res) => {
  try {
    ...
    const { stdout } = await execAsync(`curl -s -X POST "http://localhost:18789/api/sessions/send" -H "Authorization: Bearer [REDACTED]" -H "Content-Type: application/json" -d '${payload}'`);
    res.json(JSON.parse(stdout));
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
...
app.get('/api/cron', async (req, res) => {
  try {
    const { stdout } = await execAsync('curl -s "http://localhost:18789/api/cron/list?includeDisabled=true" -H "Authorization: Bearer [REDACTED]"');
    res.json(JSON.parse(stdout));
  } catch (e) {
    res.json({ jobs: [] });
  }
...
app.post('/api/cron/:id/toggle', async (req, res) => {
  try {
    ...
    const { stdout } = await execAsync(`curl -s -X POST "http://localhost:18789/api/cron/update" -H "Authorization: Bearer [REDACTED]" -H "Content-Type: application/json" -d '${payload}'`);
    res.json(JSON.parse(stdout));
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
...
app.post('/api/scheduler/run/:id', async (req, res) => {
  try {
    ...
    const { stdout } = await execAsync(`curl -s -X POST "http://localhost:18789/api/cron/run" -H "Authorization: Bearer [REDACTED]" -H "Content-Type: application/json" -d '${payload}'`);
    res.json(JSON.parse(stdout));
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
