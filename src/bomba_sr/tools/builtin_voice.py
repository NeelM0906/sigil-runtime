...
def _http_json(
    ...
    *,
    ...
    try:
        ...
        raise ValueError(f"Bland API request failed (HTTP {exc.code}){detail}") from exc
    except urllib.error.URLError as exc:
        raise ValueError(f"Bland API request failed: {exc.reason}") from exc
    if not body.strip():
        return {}
    try:
        return json.loads(body)
    except json.JSONDecodeError as exc:
        raise ValueError("Bland API returned invalid JSON") from exc
